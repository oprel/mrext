...
| LaunchSeq | Zaparoo playlists can sequence or shuffle media | No direct timed regression loop with fixed delay and offset |
| PocketBackup | Use **Pocket Backup** in Update All's **Analogue Pocket** menu | Standalone mrext installation is deprecated; Update All can run the backup on demand or during its normal update run |
...
The standalone mrext PocketBackup installation is deprecated because Pocket Backup is built into [Update All](https://github.com/theypsilon/Update_All_MiSTer). Open Update All's settings and use the **Analogue Pocket** menu. The backup can run on demand or as part of a normal Update All run.

This is a move to Update All, not a Zaparoo replacement. Zaparoo device backups serve a different purpose and do not copy an Analogue Pocket over USB.
